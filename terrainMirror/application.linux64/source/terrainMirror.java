...
public class terrainMirror extends PApplet {
...
float[][] terrain; 
float smoothingFactor = 0.05f;                  //makes the fft go slowlier so that its easier for the eye to see
float[][] sum;
float barWidth;
...
public void draw() {
 ...
 //translate(width/4,height/2,-100);
  ...
  translate(width/4+(width/2)*amp.analyze(),height*(1-amp.analyze()*0.05f)/2);                             //amp goes between 0 and 1, creates distorsion illusion                          
  rotateX(amp.analyze()*PI/4); 
  rotateX(PI/5+PI*(1.05f-amp.analyze())/4);                                                //TODO mirror fft in the same loop so that the low frequencies stay in the middle. x symmetry  
